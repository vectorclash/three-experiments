(function () {
  ...
  function init() {
    ...
    for(var s = 0; s < shapeNum; s++) {
      ...
      geometry = new THREE.TetrahedronGeometry(10, 4);
  ...
  function render() {
    var delta = clock.getDelta(),
		    time = clock.getElapsedTime() * 0.0005;
    ...
    for(var s = 0; s < shapes.children.length; s++) {
      ...
      if(!resetting) {
        ...
        //shape.material.color.lerp(new THREE.Color(s/shapes.children.length, 0.5, noise.simplex3(s, time, total)), 0.2);

        for ( var i = 0; i < shape.geometry.vertices.length; i ++ ) {
          var soundModifier = byteArray[i] * 0.009;
          if(soundModifier < 0.2) {
            soundModifier = 0.2;
          }
          shape.geometry.vertices[i].x = shape.verticesOrigin[i].x * soundModifier;
          shape.geometry.vertices[i].y = shape.verticesOrigin[i].y * soundModifier;
          shape.geometry.vertices[i].z = shape.verticesOrigin[i].z * soundModifier;
    		}
      }
  ...
  function onClick(event) {
    if(resetting) {
      ...
      for(var s = 0; s < shapes.children.length; s++) {
        ...
        for ( var i = 0; i < shape.geometry.vertices.length; i ++ ) {
          TweenMax.to(shape.geometry.vertices[i], 0.5, {x:shape.verticesOrigin[i].x, y:shape.verticesOrigin[i].y, z:shape.verticesOrigin[i].z, ease:Bounce.easeInOut, delay:i*0.001});
      	}
      }
    }
  }
